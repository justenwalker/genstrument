...
// TraceMyWrappedInterface adds APM traces around the wrapped example.MyWrappedInterface using the provided tracer.
func TraceMyWrappedInterface(tracer genstrument.Tracer, wrapped example.MyWrappedInterface) example.MyWrappedInterface {
    return &unexportedMyWrappedInterface{
        tracer: tracer,
        wrapped: wrapped,
    }
}

type unexportedMyWrappedInterface struct {
    wrapped example.MyWrappedInterface
    tracer genstrument.Tracer
}

func (w *unexportedMyWrappedInterface) Foo(ctx context.Context) (ret0 string) {
    // Start Span
    var span genstrument.Span
    ctx, span = w.tracer.StartSpan(ctx,"example.MyWrappedInterface:Foo")

    // call Wrapped Function
    ret0 =  w.wrapped.Foo(ctx)
...
func TraceGenericService[T any,PT cmp.Ordered](tracer genstrument.Tracer, wrapped example.GenericService[T,PT]) *TracedGenericService[T,PT] {
...
func InstrumentComplexService(tracer genstrument.Tracer, wrapped example.ComplexService) *InstrumentedComplexService {
